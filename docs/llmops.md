...
Colab fine-tune sonrası aynı oturumda **geçici inferans sunucusu** olabilir. Notebook FastAPI + transformers ile `0.0.0.0:8000`'de OpenAI uyumlu API sunar (`/v1/models`, `/v1/chat/completions` — `/v1/responses` yok). Inferans tüneli: **named** (`cloudflared tunnel run --token`, sabit alt alan) tercih; token yoksa **quick** (`trycloudflare`). Token yalnızca Colab secret/env — Cherry yalnızca public HTTPS URL saklar (`setColabInferenceUrl`, LLM yönetici UI). Completer `colab-tunnel` kanalını kullanır; OpenCode aynı URL’yi `cherry-colab` + `@ai-sdk/openai-compatible` ile alır (built-in `openai` `/v1/responses` 404 verir). GDPR katmanı aktif kalır. Sağlık kontrolü 30 saniyede bir yoklar (timeout + retry); bağlantı kopunca varsayılan endpoint'e düşer. Token sohbette göründüyse Zero Trust’te döndür. **Bu üretim inferansı değil** — Colab oturumu kapanınca biter.
