...
Colab fine-tune sonrası aynı oturumda **geçici inferans sunucusu** olabilir. Notebook FastAPI + transformers ile `0.0.0.0:8000`'de OpenAI uyumlu API sunar (`/v1/models`, `/v1/chat/completions` — `/v1/responses` yok). Inferans tüneli: **named** (`cloudflared tunnel run --token`, sabit alt alan) tercih; token yoksa **quick** (`trycloudflare`). Token yalnızca Colab secret/env — Cherry **işçi A ve B için ayrı** public HTTPS URL saklar (`setColabInferenceUrl(slot, url)`, LLM yönetici iki kart). Router yuvayı seçince o yuvanın tüneli kullanılır. Completer `colab-tunnel` kanalını kullanır; OpenCode aynı yuva URL’sini `cherry-colab` + `@ai-sdk/openai-compatible` ile alır. GDPR katmanı aktif kalır. Sağlık kontrolü 30 saniyede bir yoklar; bağlantı kopunca o yuva varsayılan endpoint’e düşer. Token sohbette göründüyse Zero Trust’te döndür. **Bu üretim inferansı değil** — Colab oturumu kapanınca biter.
